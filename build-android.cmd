...
REM 心流日记 Android 混合脚本单文件版 (cmd + PowerShell)
REM 用法:
REM   调试包:   build-android.cmd debug [deviceSerial]
REM   发布包:   build-android.cmd release [deviceSerial]
REM   导出AAB:  build-android.cmd aab
REM   默认 debug: 直接双击或运行 build-android.cmd

set MODE=%1
set DEVICE=%2
if "%MODE%"=="" set MODE=debug
if not "%MODE%"=="" if "%MODE:~0,1%"=="-" set MODE=debug

set BUILD_MODE=%MODE%
set BUILD_DEVICE=%DEVICE%

set "SCRIPT_SELF=%~f0"
set "TMPPS=%CD%\.tmp_build_android.ps1"
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$ErrorActionPreference='Stop'; $full = Get-Content -Raw -Encoding UTF8 '%~f0'; $parts = [regex]::Split($full, '(?m)^#\s*endbatch\s*$'); [System.IO.File]::WriteAllText('%TMPPS%', $parts[1], [System.Text.Encoding]::UTF8)"
if errorlevel 1 exit /b %ERRORLEVEL%
powershell -NoProfile -ExecutionPolicy Bypass -File "%TMPPS%" %*
set "PSERR=%ERRORLEVEL%"
rem del "%TMPPS%" >nul 2>&1
exit /b %PSERR%
# endbatch
