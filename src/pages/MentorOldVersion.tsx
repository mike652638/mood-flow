...
import { HeartHandshake, ShieldAlert, Send, Fingerprint } from 'lucide-react';
...
const Mentor: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatBubble[]>([
    {
      ...
      role: 'assistant',
      content:
        '您好，我是 AI 伴侣，很高兴遇见你 ✨ 无论此刻心情如何，我都在这里陪伴。想聊聊今天发生的事情吗？或者试试下方的放松练习也很不错～'
    }
  ]);
  ...
  return (
    <>
      <Header title='AI 伴侣' immersiveMode={immersiveMode} />
          ...
          <Card variant='default' padding='sm' className='overflow-hidden p-2 sm:p-3'>
            <div
              ...
              <button
                ...
                <HeartHandshake className='w-4 h-4 xl:w-5 xl:h-5 flex-shrink-0' />
        ...
        {activeTab === 'chat' && (
          <div className='space-y-6'>
              ...
              <Card variant='default' padding='sm' className='flex-1 min-w-0 order-1 sm:order-none relative'>
                <div className='relative'>
                  <textarea
                    ...
                    className={`w-full bg-white/80 dark:bg-gray-800/90 outline-none text-sm xl:text-base placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-purple-300 dark:focus:ring-purple-600 rounded-md px-3 py-2 xl:px-4 xl:py-3 transition-all duration-200 border border-gray-200 dark:border-gray-700 shadow-sm focus:shadow-md resize-none overflow-y-auto min-h-[64px] max-h-[240px] pr-12 ${
                      isKeyboardOpen ? 'fixed-input' : ''
                    }`}
                    disabled={isSending}
                  />
                  {/* 输入提示 */}
                ...
                <div
                  className='mt-2 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 sm:gap-3'
            ...
            <Card
              variant='default'
              padding='sm'
              className='overflow-hidden border border-gray-200 dark:border-gray-700 bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 shadow-sm hover:shadow-md transition-all duration-300'>
              <div className='flex items-center justify-between'>
                ...
                <button
                  ...
                  {presetOpen ? '收起' : '展开'}
                  <span className={cn('transition-transform duration-200', presetOpen ? 'rotate-180' : '')}>▼</span>
                </button>
              </div>
              <div
                className={cn(
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3 sm:gap-4'>
                  {presetGroups.map(group => (
                    <div
                      key={group.label}
                      className='rounded-xl p-3 sm:p-4 bg-white/60 dark:bg-gray-700/60 border border-gray-200 dark:border-gray-600 hover:border-purple-300 dark:hover:border-purple-600 transition-all duration-200 shadow-sm hover:shadow-md'>
                      <div className='flex items-center gap-2 mb-3 pb-2 border-b border-gray-200 dark:border-gray-600'>
                        ...
                        <span className={cn('text-sm font-semibold', group.accentClasses.text)}>{group.label}</span>
                      </div>
                      <div role='group' aria-label={`${group.label} 预设问题`} className='space-y-2'>
